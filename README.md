该项目使用的 Linux 版本为 3.10，源码地址为[Linux-3.10](https://github.com/torvalds/linux/tree/v3.10)，推荐在 wsl 的 Ubuntu24.04 中完成实验。

## 说明

**本项目以学习为唯一目的进行开源发布，使用者仅可将其作为技术参考，禁止用于学习范畴之外的任何用途（包括但不限于直接用作个人的课程设计成果）。请恪守知识产权相关规定，尊重开发者的合法权益。**

## 文档
1. 题目内容：[tasks.md](./doc-ujs/tasks.md)
2. 结题后源码的编译及测试方式：[summary.md](./doc-ujs/summary.md)
...
6. 手动去除一个模块，因为这个模块会导致fatal error：
    ...
    *如果未能打开一个 GUI 界面，大概率是是你的终端窗口太小了。*
7. 编译 busybox：
    ```bash
    make -j$(nproc)
